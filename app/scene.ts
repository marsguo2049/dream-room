...
import { buildDreamHouse, type HouseView, type HouseWindow } from "./house";
...
function placeGardenTarget(point: THREE.Vector3, out: THREE.Vector3) {
  ...
  return out.set(
    THREE.MathUtils.clamp(point.x, -10.8, 10.8),
    0,
    THREE.MathUtils.clamp(point.z, 6.65, 15.2),
  );
}
...
function createPalette(surfaces: SurfaceFactory) {
  ...
  const glazed = (color: number, gloss = 1, roughness = 0.44, surface = glazeSurface) => {
    const shared = {
      ...surface,
      color,
 ...
 */
function buildSeaWindow(scene: THREE.Scene, palette: Palette): HouseWindow[] {
  const WALL_Z = -5.35;
  const LEFT = -6.35;
  const RIGHT = 3.16;
  const HEAD = 5.6;
...
const VIEW_STATUS: Record<HouseView, string> = {
...
export type DreamRoomHandlers = {
  ...
  /** Called when the camera moves between the house and its two floors. */
...
export type { HouseView } from "./house";
...
export function createDreamRoom(mount: HTMLElement, handlers: DreamRoomHandlers): DreamRoom {
  ...
  const houseInteractive: THREE.Object3D[] = [
    dreamHouse.lawn,
  ...
  const cameraViews: Record<HouseView, { position: THREE.Vector3; target: THREE.Vector3 }> = {
    house: {
  ...
  let currentView: HouseView = "house";
  let cameraTravel = 1;
  ...
  const startAction = (action: StoryAction) => {
    if (currentView !== "ground") setView("ground");
    controls.autoRotate = false;
  ...
  const walkToGarden = (point: THREE.Vector3) => {
  ...
  const onUp = (event: PointerEvent) => {
    ...
    else if (action === "garden") walkToGarden(hit.point);
    else if (action === "window") toggleWindow(Number(hit.object.userData.windowIndex));
  ...
  const render = (now = 0) => {
    ...
    if (state === "walk-to") {
      ...
      if (p > 0.58) houseDoorTarget = -Math.PI * 0.43;
      if (phase > walkSeconds) transition("garden-exit", "门打开了，她正跨过门槛走到花园里…");
    ...
    dreamHouse.frontDoor.rotation.y = THREE.MathUtils.lerp(
      dreamHouse.frontDoor.rotation.y,
      houseDoorTarget,
      0.085,
    );
    openableWindows.forEach((window, index) => {
