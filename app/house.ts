...
export type HousePalette = {
  ...
  houseDoor: THREE.Material;
  brass: THREE.Material;
  houseTrim: THREE.Material;
 ...
 */
function buildFoot(parent: THREE.Object3D, palette: HousePalette) {
  const height = 1.82;
  const out = 0.22;
  ...
  addBox(
    parent,
    [HOUSE_HALF_X * 2 + out * 2 + 0.14, 0.17, HOUSE_HALF_Z * 2 + out * 2 + 0.14],
    [0, height, 0],
    palette.houseTrim,
    0.075,
  );
...
function buildRoof(parent: THREE.Object3D, palette: HousePalette) {
  ...
  for (const side of [-1, 1] as const) {
    ...
    const eaves = addBox(
      roof,
      [0.34, 0.62, HOUSE_HALF_Z * 2 + 1.14],
      [side * (run + 0.03), UPPER_CEILING - 0.05, 0],
      palette.roofEdge,
      0.17,
    );
...
export function buildDreamHouse(scene: THREE.Scene, palette: HousePalette): DreamHouse {
  ...
  const addFrontWall = (
    ...
    parent,
    [x2 - x1, y2 - y1, 0.38],
    [(x1 + x2) / 2, (y1 + y2) / 2, HOUSE_HALF_Z - 0.09],
    material,
    0.12,
  );
  ...
  for (const [x1, x2] of [[-6.55, -4.625], [-2.475, -1.18], [1.18, 2.475], [4.625, 6.55]] as const) {
    addFrontWall(lowerExterior, x1, x2, 0, GROUND_CEILING, palette.housePlaster);
  }
  for (const [x1, x2] of [[-4.625, -2.475], [2.475, 4.625]] as const) {
  ...
  for (const [x1, x2] of [[-6.55, -4.625], [-2.475, 2.475], [4.625, 6.55]] as const) {
  ...
  const uv = gableGeometry.attributes.uv as THREE.BufferAttribute;
