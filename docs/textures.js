// Generated from app/textures.ts by scripts/build-pages.sh. Do not edit by hand.
/**
...
function fill(painting, color, heightLevel, roughLevel) {
    const { size } = painting;
 ...
 */
function normalFromHeight(ctx, strength = 2.2) {
    const { width, height } = ctx.canvas;
    const { data } = ctx.getImageData(0, 0, width, height);
    const pixels = new Uint8Array(width * height * 4);
    const at = (x, y) => data[((((y % height) + height) % height) * width + (((x % width) + width) % width)) * 4] / 255;
...
export function createSurfaceFactory(anisotropy) {
    const dress = (texture, repeat, srgb) => {
        texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
    ...
    const resolve = (painting, repeat, normalStrength) => {
    ...
    const mottle = (ctx, size, count, colors, radius, seed) => {
    ...
    const speckle = (ctx, size, count, alpha, seed) => {
        const random = rng(seed);
        for (let i = 0; i < count; i += 1) {
     ...
     */
    const oakBoards = (tone, repeat, seed) => {
    ...
    const ceramicGlaze = () => {
        ...
        const craze = (x, y, angle, life) => {
            ...
            if (random() > 0.72)
                craze(nx, ny, angle + (random() - 0.5) * 2.6, life - 2);
        };
    ...
    const finishVariation = (base, blotches, repeat, seed) => {
        const size = 512;
    ...
    const radialFalloff = (inner, outer, midpoint = 0.45) => {
