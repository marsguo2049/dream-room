...
const PACE = 1.35;
const paceSeconds = (from, to) => THREE.MathUtils.clamp(from.distanceTo(to) / PACE, 1.1, 6);
 ...
 */
function placeWalkTarget(point, out) {
 ...
 */
function buildSeaWindow(scene, palette) {
    const WALL_Z = -5.35;
    const LEFT = -6.35;
    const RIGHT = 3.16;
    const HEAD = 5.6;
...
function buildOcean(scene, palette, surfaces) {
    ...
    const sky = new THREE.Mesh(new THREE.PlaneGeometry(54, 31), new THREE.MeshBasicMaterial({
        map: surfaces.seaSky(),
        toneMapped: false,
        fog: false,
        side: THREE.DoubleSide,
...
function legoBrick(parent, studs, position, material, rotation = 0) {
    ...
    for (let i = 0; i < studs[0]; i += 1) {
        for (let j = 0; j < studs[1]; j += 1) {
            const stud = new THREE.Mesh(new THREE.CylinderGeometry(STUD_RADIUS, STUD_RADIUS, STUD_PITCH * 0.22, 16), material);
            stud.position.set(-width / 2 + STUD_PITCH * (i + 0.5), BRICK_HEIGHT + STUD_PITCH * 0.09, -depth / 2 + STUD_PITCH * (j + 0.5));
...
function buildLego(parent, base) {
    ...
    const bricks = pieces.map((piece) => {
        ...
        brick.userData.builtQuaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(piece.built[3], piece.built[4], piece.built[5]));
        brick.userData.loosePosition = new THREE.Vector3(piece.loose[0], piece.loose[1], piece.loose[2]);
        brick.userData.looseQuaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(piece.loose[3], piece.loose[4], piece.loose[5]));
        return brick;
    });
    return { group, bricks };
}
/** Drives the pile between built (0) and scattered (1). */
function applyLego(pile, amount) {
    const t = ease(amount);
    pile.bricks.forEach((brick, index) => {
        const built = brick.userData.builtPosition;
        const loose = brick.userData.loosePosition;
        ...
        brick.quaternion.slerpQuaternions(brick.userData.builtQuaternion, brick.userData.looseQuaternion, t);
    });
}
 ...
 */
function reachTo(limb, target, upper, lower) {
    ...
    const elbow = Math.PI -
        Math.acos(THREE.MathUtils.clamp((upper * upper + lower * lower - distance * distance) / (2 * upper * lower), -1, 1));
    const offset = Math.acos(THREE.MathUtils.clamp((upper * upper + distance * distance - lower * lower) / (2 * upper * distance), -1, 1));
...
export function createDreamRoom(mount, handlers) {
    ...
    const heldPencil = new THREE.Mesh(new THREE.CylinderGeometry(0.021, 0.016, 0.32, 12), pencilWood);
    ...
    const render = () => {
        ...
        if (state === "walk-to") {
            ...
            reachTarget.set(0.1 + Math.sin(phase * 5.4) * 0.13, 1.4, 0.68 + Math.sin(phase * 2.7) * 0.05);
            reachTo(girl.rightArm, reachTarget, BODY.upperArm, BODY.lowerArm);
